...
const products = computed(() => store.getters['products/data'].filter((e) => e.product.sub_category[0].category.name == 'Женские'))
</script>
...
<template>
    <section>
        ...
        <template v-else>
            <ProductItem v-for="item in products.slice(0, 2)" :data="item" image="Rectangle89" />
