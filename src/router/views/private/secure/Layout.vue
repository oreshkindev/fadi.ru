...
<template>
    <header>
        <nav :class="{ visible: isVisible }">
            ...
            <i class="icon-menu" @click="isVisible = !isVisible"></i>
            <i class="icon-dots"></i>

            <router-link to="/secure/users">База данных</router-link>

            <router-link to="/secure/settings">Настройки</router-link>

            <router-link to="/secure/redactor">Редактор</router-link>

            <router-link to="/logout">Выйти</router-link>

            <router-link to="/secure/users" custom v-slot="{ navigate }">
                <picture @click="navigate" @keypress.enter="navigate" role="link">
                    <source srcset="@/assets/images/Rectangle452.webp" type="image/webp" />
                    <img src="@/assets/images/Rectangle452.jpg" decoding="async" alt="Давайте знакомиться" loading="lazy" />
                </picture>

                <!-- <i class="icon-user" @click="navigate" @keypress.enter="navigate" role="link"></i> -->
            </router-link>
